 ...
 *
 * The read tools in this server hand back App Store Connect's payload close to
 ...
 * honestly say which of App Store Connect's fields matter and which are noise.
 *
 ...
 */
export const ASC_VIEWS = ['compact', 'full'] as const;

const NOTE =
  'compact strips image/avatar URLs from the response; "full" returns App Store Connect\'s payload untouched. ' +
  'No field projection: this server has no verified record of which App Store Connect fields matter, and inventing ' +
  'one would risk dropping a field a caller needs.';

/** The `view` parameter every read tool in this server takes. */
export const viewArg = (): ReturnType<typeof viewParam> => viewParam(ASC_VIEWS, { note: NOTE });
...
export function viewResponse(view: string | undefined, data: unknown): ReturnType<typeof minifiedResult> {
  const rung: View = resolveView(view, ASC_VIEWS);
